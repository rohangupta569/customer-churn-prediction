...
def create_evaluation_plots(
    ...
    roc_display = RocCurveDisplay.from_estimator(model, X_test, y_test, ax=axes[1])
    roc_display.line_.set_color("#2962ff")
    axes[1].plot([0, 1], [0, 1], "--", color="grey", linewidth=1)
    axes[1].set_title("ROC curve")
    precision_recall_display = PrecisionRecallDisplay.from_estimator(model, X_test, y_test, ax=axes[2])
    precision_recall_display.line_.set_color("#e45756")
